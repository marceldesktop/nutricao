...
class funcionarios extends model {
    ...
    public function getListTodosUsuarios (){
        
        if(!empty($pesquisa)){
            $sql = "SELECT * FROM funcionarios WHERE nome ='".$pesquisa." OR email ='".$pesquisa."' ";
            $sql = $this->db->quey($sql);
            if($sql->rowCount ()>0){
                $array = $sql->fetchAll();
                return $array;
            }else{
    ...
    public function cadastrarFunc($nome, $email, $senha, $sexo, $celular) {
        $data=date(Y-m-d);
        $sql = "INSERT INTO funcionarios SET nome='" . $nome . "', email='" . $email . "', senha='" . $senha . "', sexo='" . $sexo . "',celular='" . $celular . "',status='Inativo',data=NOW() ";
        $sql = $this->db->query($sql);
