...
//metodo ver se usuario esta logado ou se nao direciona para logar 
    ...
    public function pacienteExisteCpf($cpf) {
    ...
    public function getListTodosPacientes ($pesquisa){
        $array=array();
        if(!empty($pesquisa)){
            $sql = "SELECT * FROM pacientes WHERE nome LIKE'%".$pesquisa."%' OR email LIKE'%".$pesquisa."%' ";
            $sql = $this->db->query($sql);
            if($sql->rowCount() > 0){
                $array = $sql->fetchAll();
    ...
    public function cadastrarPaciente($cpf, $nome, $sexo, $celular, $endereco, $bairro, $cidade, $estado, $cep) {
       
        $sql = "INSERT INTO pacientes SET cpf='".$cpf."',nome='" . $nome . "', sexo='" . $sexo . "',celular='" . $celular . "',endereco='" . $endereco . "',bairro='" . $bairro . "',cidade='" . $cidade . "',estado='" . $estado . "',cep='" . $cep . "'status='Inativo', data = NOW() ";
